#tag Class
Protected Class Scan4TunesThread
Inherits Thread
	#tag Event
		Sub Run()
	...
	#tag Method, Flags = &h0
		Sub ScanFolder(theFolder as FolderItem)
		  Dim sql, Band, Album, Title, Artist As  string
		  Var tracknum, Year As integer
		  dim f as folderitem
		  'dim mediaFile As EKMediaFile
		    ...
		    if f <> nil then
		      if f.Exists then
		        ...
		        if f.IsFolder then
		          ...
		          Case ".mp3", ".m4a", ".m4u", ".aac"
		            
		            //Check if the file is already added
		            Var rs As RowSet
		            Try
		              rs = tMyDB.SelectSQL("SELECT TuneID FROM tblTunes WHERE Location = ?", f.NativePath)
		              if rs.RowCount=0 then
		                ...
		                Try
		                  //Get ID3-tags, if exists update tune data
		                  mediaFile = New EKMediaFile(f, false)
		                  
		                  
		                  Title=mediaFile.ID3Tags.Lookup("Title", f.Name).StringValue//.DefineEncoding(Encodings.UTF16BE)
		                  theEncoding = Guessencoding(Title)
		                  Title=Title.DefineEncoding(theEncoding).ConvertEncoding(Encodings.UTF8)
		                  
		                  
		                  Band=mediaFile.ID3Tags.Lookup("Band", Artist).StringValue.DefineEncoding(theEncoding)
		                  if Band = "" then
		                    Band=mediaFile.ID3Tags.Lookup("Artist", f.Name).StringValue.DefineEncoding(theEncoding)
		                  end if
		                  theEncoding = Guessencoding(Band)
		                  Band=Band.DefineEncoding(theEncoding).ConvertEncoding(Encodings.UTF8)
		                  
		                  tracknum=mediaFile.ID3Tags.Lookup("tracknum", f.Name).StringValue.ToInteger//DefineEncoding(theEncoding)
		                  
		                  Album=mediaFile.ID3Tags.Lookup("Album", theFolder.Name).StringValue//.DefineEncoding(theEncoding)
		                  theEncoding = Guessencoding(Album)
		                  Album=Album.DefineEncoding(theEncoding).ConvertEncoding(Encodings.UTF8)
		                  
		                  Year=mediaFile.ID3Tags.Lookup("Year", "").StringValue.ToInteger//DefineEncoding(theEncoding)
		                  
		                Catch err As OutOfMemoryException
		                  'theEncoding = Encodings.ASCII
		                End Try
		                
		                if Title.Bytes>128 then //Check for overflow of binary data, tune ID3 titles are never more than 128 chars long
		                  theEncoding = Guessencoding(f.Name)
		                  Title=f.Name.DefineEncoding(theEncoding).ConvertEncoding(Encodings.UTF8)
		                end if
		                
		                Title=GetCleanUTFText(Title)
		                Band=GetCleanUTFText(Band)
		                Album=GetCleanUTFText(Album)
		                
		                //Fallback if ID3 tag extraction failed, we presume file path of .../band/album/tune.xxx
		                if Title = "" then
		                  theEncoding = Guessencoding(f.Name)
		                  Title=f.Name.DefineEncoding(theEncoding).ConvertEncoding(Encodings.UTF8)
		                end if
		                if Album = "" or Album = " " then
		                  Album=f.Parent.Name.DefineEncoding(theEncoding)
		                end if
		                if Band = "" then
		                  theEncoding = Guessencoding(f.Name)
		                  Band=f.Parent.Parent.Name.DefineEncoding(theEncoding).ConvertEncoding(Encodings.UTF8)
		                end if
		                
		                
		                if Title<>"" then
		                  //Update DB with data
		                  sql = "INSERT INTO tblTunes (Band, Album, Title, tracknum, Year, Location) VALUES(?, ?, ?, ?, ?, ?)"
		                  Try
		                    tMyDB.ExecuteSQL(sql, Band, Album, Title, tracknum.ToString, Year.ToString, f.NativePath)
		                    'rowID = MyDB.LastRowID
		                  Catch error As DatabaseException
